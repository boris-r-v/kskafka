...
MongooseConnection::MongooseConnection( const MongooseConnection& _mc ): m_conn( _mc.m_conn )
{
}
...
/*
FIXME: to known how in 3.3 ver send autorizationrequest
...
MongooseRequest::MongooseRequest(struct mg_connection *conn ):
  m_conn(conn), m_info( mg_get_request_info( conn ) )
{
}
...
{
    return std::string( mg_get_log_message ( m_conn ) );
}
...
{
    return mg_get_reply_status_code( m_conn );
}
...
const mg_request_info* MongooseRequest::getInfo() const
{
    return m_info;
}
...
void MongooseResponse::addHeaderValue(const std::string &name, const std::string &value)
{
    ...
    if (cit != m_headerValuesIndex->end()) {
        ...
        ;// FIXMEm_headerValuesIndex->insert(std::make_pair<std::string, int>(name, idx));
    }
    // FIXMEm_headerValues->push_back(std::make_pair<std::string, std::string>(name, value));
}
...
void MongooseResponse::setHeaderValue(const std::string &name, const std::string &value)
{
    ...
    if (cit != m_headerValuesIndex->end()) {
        ...
        int idx = m_headerValues->size();
    // FIXME    m_headerValuesIndex->insert(std::make_pair<std::string, int>(name, idx));
    // FIXME    m_headerValues->push_back(std::make_pair<std::string, std::string>(name, value));
    }
}
...
//-----------------------------------------------------------------------
static void *LocalMongooseEventHandler( enum mg_event eventCode, struct mg_connection *conn )
{
    return static_cast<MongooseServer*> ( mg_get_user_data(conn) ) -> handleEvent(eventCode, conn );
}
...
void MongooseServer::start()
{
    ...
    m_ctx = mg_start( LocalMongooseEventHandler, this, prepareOptions() );
...
bool MongooseServer::on_event(ServerHandlingEvent eventCode, MongooseConnection &connection, const MongooseRequest &request, MongooseResponse &response)
{
    return false;
}

void *MongooseServer::handleEvent(ServerHandlingEvent eventCode, struct mg_connection *conn )
{
    ...
    return on_event(eventCode, *connection, *request, *response) ? const_cast<char*>("") : NULL;
}
